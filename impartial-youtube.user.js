...
var location = window.location; // for semistandard's sake
document.onreadystatechange = () => {
  if (document.readyState === 'complete') window.setInterval(impartial, 1000);
};

function impartial () {
  // hide views counts on home page, search page, trending page and subscription page
  if (location.pathname !== '/watch') {
    Array.prototype.forEach.call(document.querySelectorAll('.yt-lockup-meta-info'), (v) => {
      if (
        location.pathname === '/' ||
  ...
  // tweaks on video page
    ...
    Array.prototype.forEach.call(document.querySelectorAll('.view-count'), (v) => {
    ...
    var commentsHeader = document.querySelector('h2.comment-section-header-renderer');
    var sortMenu = document.querySelector('.comment-section-sort-menu');
    var comments = document.querySelector('#comment-section-renderer-items');
    var loadMoreBtn = document.querySelector('.comment-section-renderer-paginator');
    
    var commentsHeaderOriginalInnerHTML = commentsHeader.innerHTML;
    ...
    var showCommentsBtn = document.createElement('button');
  ...
  // hide views counts and subscribers count on user's page, user's videos page and channels page
